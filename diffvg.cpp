...
static long long g_ellipse_forward_pixel_calls = 0;
static long long g_ellipse_backward_pixel_calls = 0;

void reset_ellipse_wendland_timing() {
    g_ellipse_forward_time_ms = 0.0;
    g_ellipse_backward_time_ms = 0.0;
    g_ellipse_forward_pixel_calls = 0;
    g_ellipse_backward_pixel_calls = 0;
}

void print_ellipse_wendland_timing() {
    printf("---- render_ellipse_wendland timing ----\n");
    printf("Forward:  %.3f ms total, %lld pixel-calls, %.6f ms/pixel\n",
           g_ellipse_forward_time_ms, g_ellipse_forward_pixel_calls,
           g_ellipse_forward_pixel_calls > 0 ? g_ellipse_forward_time_ms / g_ellipse_forward_pixel_calls : 0.0);
    printf("Backward: %.3f ms total, %lld pixel-calls, %.6f ms/pixel\n",
           g_ellipse_backward_time_ms, g_ellipse_backward_pixel_calls,
           g_ellipse_backward_pixel_calls > 0 ? g_ellipse_backward_time_ms / g_ellipse_backward_pixel_calls : 0.0);
}
...
void render_ellipse_wendland(const EllipseWendlandField &field,
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            g_ellipse_forward_time_ms += std::chrono::duration<double, std::milli>(fwd_end - fwd_start).count();
            g_ellipse_forward_pixel_calls++;
            ...
            if (d_render_image.get() != nullptr) {
                ...
                g_ellipse_backward_time_ms += std::chrono::duration<double, std::milli>(bwd_end - bwd_start).count();
                g_ellipse_backward_pixel_calls++;
            }
        }
    }
}
