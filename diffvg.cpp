...
// ============================================================
// Boxed variant: same Gaussian RBF kernel, but ellipses are bucketed into
// a tile grid so each pixel only checks nearby ellipses instead of all N.
// Kept as a separate function from render_ellipse_gaussian so the original
// stays untouched and validated.
// ============================================================
static double g_ellipse_gaussian_boxed_forward_time_ms = 0.0;
static double g_ellipse_gaussian_boxed_backward_time_ms = 0.0;
static long long g_ellipse_gaussian_boxed_forward_pixel_calls = 0;
static long long g_ellipse_gaussian_boxed_backward_pixel_calls = 0;
void reset_ellipse_gaussian_boxed_timing() {
    g_ellipse_gaussian_boxed_forward_time_ms = 0.0;
    g_ellipse_gaussian_boxed_backward_time_ms = 0.0;
    g_ellipse_gaussian_boxed_forward_pixel_calls = 0;
    g_ellipse_gaussian_boxed_backward_pixel_calls = 0;
}
void print_ellipse_gaussian_boxed_timing() {
    printf("---- render_ellipse_gaussian_boxed timing ----\n");
    printf("Forward:  %.3f ms total, %lld pixel-calls, %.6f ms/pixel\n",
           g_ellipse_gaussian_boxed_forward_time_ms, g_ellipse_gaussian_boxed_forward_pixel_calls,
           g_ellipse_gaussian_boxed_forward_pixel_calls > 0 ? g_ellipse_gaussian_boxed_forward_time_ms / g_ellipse_gaussian_boxed_forward_pixel_calls : 0.0);
    printf("Backward: %.3f ms total, %lld pixel-calls, %.6f ms/pixel\n",
           g_ellipse_gaussian_boxed_backward_time_ms, g_ellipse_gaussian_boxed_backward_pixel_calls,
           g_ellipse_gaussian_boxed_backward_pixel_calls > 0 ? g_ellipse_gaussian_boxed_backward_time_ms / g_ellipse_gaussian_boxed_backward_pixel_calls : 0.0);
}
py::tuple get_ellipse_gaussian_boxed_timing() {
    return py::make_tuple(g_ellipse_gaussian_boxed_forward_time_ms, g_ellipse_gaussian_boxed_forward_pixel_calls,
                           g_ellipse_gaussian_boxed_backward_time_ms, g_ellipse_gaussian_boxed_backward_pixel_calls);
}
...
constexpr int GAUSSIAN_TILE_SIZE = 64; // pixels per tile side, change if needed

struct EllipseGaussianTileGrid {
    int tile_size;
    int num_tiles_x, num_tiles_y;
    std::vector<std::vector<int>> tiles; // ascending ellipse index per tile

    EllipseGaussianTileGrid(const EllipseGaussianField &field, int width, int height, int tile_size)
        : tile_size(tile_size) {
...
void render_ellipse_gaussian_boxed(const EllipseGaussianField &field,
                                   ptr<float> background_image,
                                   ptr<float> render_image,
                                   ptr<float> d_render_image,
                                   ptr<float> d_positions,
                                   ...
                                   int height) {
    const float sigma2 = GAUSSIAN_SIGMA * GAUSSIAN_SIGMA;
    EllipseGaussianTileGrid grid(field, width, height, GAUSSIAN_TILE_SIZE);
    ...
    for (int y = 0; y < height; y++) {
        for (int x = 0; x < width; x++) {
            ...
            auto fwd_end = std::chrono::high_resolution_clock::now();
            g_ellipse_gaussian_boxed_forward_time_ms += std::chrono::duration<double, std::milli>(fwd_end - fwd_start).count();
            g_ellipse_gaussian_boxed_forward_pixel_calls++;
            ...
            if (d_render_image.get() != nullptr) {
                ...
                auto bwd_end = std::chrono::high_resolution_clock::now();
                g_ellipse_gaussian_boxed_backward_time_ms += std::chrono::duration<double, std::milli>(bwd_end - bwd_start).count();
                g_ellipse_gaussian_boxed_backward_pixel_calls++;
            }
        }
    }
}
...
PYBIND11_MODULE(diffvg, m) {
    ...
    m.def("render_ellipse_gaussian_boxed", &render_ellipse_gaussian_boxed, "");
    m.def("reset_ellipse_gaussian_boxed_timing", &reset_ellipse_gaussian_boxed_timing, "");
    m.def("print_ellipse_gaussian_boxed_timing", &print_ellipse_gaussian_boxed_timing, "");
    m.def("get_ellipse_gaussian_boxed_timing", &get_ellipse_gaussian_boxed_timing, "");
