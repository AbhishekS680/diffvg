# n_vs_time_gaussian_boxed.py
#
# Measures forward+backward render time for the tile-grid accelerated
# Gaussian RBF renderer as N (number of ellipses) increases -- one
# forward+backward pass per N value, no training. Paired with
# n_vs_time_gaussian.py to compute the boxed-vs-plain speedup reported
...
#
# Ellipse semi-axes are fixed at log(0.15), matching every other
# n_vs_time_*.py script in the Wendland/Gaussian family, so no primitive
# is confounded by a different average ellipse size during the sweep.
import argparse
import os
import time
import torch
...
with open(f'{OUTDIR}/n_vs_time_gaussian_boxed.txt', 'w') as f:
...
print(f'saved {OUTDIR}/n_vs_time_gaussian_boxed.txt')
...
ax.set_title('Render time vs N (Gaussian, boxed/tile-grid)')
ax.legend()
plt.savefig(f'{OUTDIR}/n_vs_time_gaussian_boxed.png', bbox_inches='tight', dpi=150)
plt.close(fig)
print(f'saved {OUTDIR}/n_vs_time_gaussian_boxed.png')