...
#
# Measures forward+backward render time for Wendland C2 as N (number of
# ellipses) increases -- one forward+backward pass per N value, no
...
# every N in the sweep -- this value must match n_vs_time_gaussian.py's
# equivalent so the two primitives' timings are compared at the same
# ellipse size, not confounded by different average coverage per ellipse.
import argparse
import os
import time
import torch
...
with open(f'{OUTDIR}/n_vs_time_wendland.txt', 'w') as f:
...
print(f'saved {OUTDIR}/n_vs_time_wendland.txt')
...
ax.legend()
plt.savefig(f'{OUTDIR}/n_vs_time_wendland.png', bbox_inches='tight', dpi=150)
plt.close(fig)
print(f'saved {OUTDIR}/n_vs_time_wendland.png')