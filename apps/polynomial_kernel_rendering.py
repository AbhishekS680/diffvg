...
#
# Anisotropic ellipse renderer using a learnable global polynomial
# kernel instead of a fixed formula like Wendland's:
#   f(t) = a*t^4 + b*t^3 + c*t^2 + d*t + e
# Coefficients (a..e) start matching the Wendland C2 shape and are
# optimized jointly with each ellipse's position/shape/colour.
#
# NOT part of the core four-primitive comparison (Wendland / Gaussian /
# Shepard / Triangle Soup) used in the report -- this is an additional,
# exploratory primitive: instead of choosing a falloff function by
# hand, let gradient descent discover one directly.
#
# Usage:
#   python polynomial_kernel_rendering.py --image imgs/cat.png --n 1000 --iters 200
#
# Args:
...
#
# poly_coeffs uses a much lower learning rate (1e-4) than
# position/shape/colour (1e-2), since the polynomial is more sensitive
# to coefficient changes -- a small coefficient shift can swing the
# kernel's shape drastically at high powers of t.
import argparse
import os
import torch
import numpy as np
...
OUTDIR = 'results/polynomial_kernel_rendering'
...
# Polynomial coefficients [a,b,c,d,e], initialized to match (1-t)^4,
# a shape close to Wendland's, so training starts from a familiar kernel
...
for t in range(iters):
    ...
    coeff_history.append(poly_coeffs.detach().mean(dim=0).clone().numpy())
    loss.backward()
    ...
    with torch.no_grad():
        ...
        # theta and poly_coeffs are intentionally unclamped
...
with open(f'{OUTDIR}/final_coeffs.txt', 'w') as f:
    names = ['a (t^4)', 'b (t^3)', 'c (t^2)', 'd (t^1)', 'e (t^0)']
    coeffs_np = poly_coeffs.detach().numpy()
...
# --- Coefficient trajectories over training ---
...
ax.legend()
plt.savefig(f'{OUTDIR}/coefficient_trajectories.png', bbox_inches='tight', dpi=150)
plt.close(fig)
print('saved coefficient_trajectories.png')

# --- Kernel shape: initial vs learned vs Wendland reference ---
t_range = torch.linspace(0, 1, 200)
final_coeffs = poly_coeffs.detach().mean(dim=0)

def eval_poly(coeffs, t):
    a, b, c, d, e = coeffs
    return (a * t**4 + b * t**3 + c * t**2 + d * t + e).clamp(0.0, 1.0)
...
# --- Labeled ellipses with per-ellipse effective boundary ---
# Each ellipse has its own 5 coefficients, so the boundary where its
# kernel drops to ALPHA_CUTOFF differs per ellipse -- solved via
# vectorized bisection across all N at once, rather than one shared t
# like Wendland's fixed formula.
...
for _ in range(50):
    mid = (lo + hi) / 2
    val = np.clip(a_c*mid**4 + b_c*mid**3 + c_c*mid**2 + d_c*mid + e_c, 0.0, 1.0)
