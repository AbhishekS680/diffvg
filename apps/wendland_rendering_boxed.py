...
fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
plt.savefig('results/wendland_rendering_boxed/focus_weight_map.png', bbox_inches='tight', dpi=150)
plt.close(fig)
print('saved focus_weight_map.png')

focus_loss_history = []
os.makedirs('results/wendland_rendering_boxed/focus_iters', exist_ok=True)
...
for t in range(FOCUS_ITERS):
    ...
    pydiffvg.imwrite(img.detach().clamp(0, 1).cpu(), 'results/wendland_rendering_boxed/focus_iters/iter_{}.png'.format(t), gamma=1.0)

print(f'final focus loss: {focus_loss.item():.4f}')
with open('results/wendland_rendering_boxed/focus_final_loss.txt', 'w') as f:
...
plt.savefig('results/wendland_rendering_boxed/focus_loss_curve.png', bbox_inches='tight', dpi=150)
plt.close(fig)
...
a_px = torch.exp(log_a) * canvas_width
b_px = torch.exp(log_b) * canvas_width
final_focused = pydiffvg.EllipseWendlandBoxedRenderFunction.apply(positions_px, colors, a_px, b_px, theta, None, canvas_width, canvas_height)
final_focused = final_focused.clamp(0, 1)
pydiffvg.imwrite(final_focused.detach().cpu(), 'results/wendland_rendering_boxed/final_focused.png', gamma=1.0)
...
ax.axis('off')
fig.colorbar(im, ax=ax, fraction=0.046, pad=0.04)
plt.savefig('results/wendland_rendering_boxed/error_heatmap_focused.png', bbox_inches='tight', dpi=150)
plt.close(fig)
...
with open('results/wendland_rendering_boxed/focus_summary.txt', 'w') as f:
...
plt.savefig('results/wendland_rendering_boxed/focus_comparison.png', bbox_inches='tight', dpi=150)
plt.close(fig)
