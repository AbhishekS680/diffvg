# n_vs_time_trianglesoup.py
#
# Measures forward+backward render time for Triangle Soup as N (number
# of triangles) increases -- one forward+backward pass per N value, no
...
#
# SOFTNESS is fixed at 0.5, matching n_vs_time_trianglesoup_boxed.py --
# this measures raw forward/backward cost at one softness value, not
# convergence, so no annealing is needed.
import argparse
import os
import time
import torch
...
with open(f'{OUTDIR}/n_vs_time_trianglesoup.txt', 'w') as f:
...
print(f'saved {OUTDIR}/n_vs_time_trianglesoup.txt')
...
ax.set_title('Render time vs N (Triangle Soup)')
ax.legend()
plt.savefig(f'{OUTDIR}/n_vs_time_trianglesoup.png', bbox_inches='tight', dpi=150)
plt.close(fig)
print(f'saved {OUTDIR}/n_vs_time_trianglesoup.png')