...
#
# Measures forward+backward render time for Gaussian RBF as N (number
# of ellipses) increases -- one forward+backward pass per N value, no
# training. Feeds into the "Performance Comparison" section of the
# report (Table speedup, Figure n_vs_time_combined).
#
# Usage:
...
#
# Ellipse semi-axes are fixed at a representative size (log(0.15)) for
# every N in the sweep -- matches n_vs_time_wendland.py and
# n_vs_time_gaussian_boxed.py exactly, so timings across primitives
# aren't confounded by different average ellipse sizes.
import argparse
import os
import time
import torch
...
with open(f'{OUTDIR}/n_vs_time_gaussian.txt', 'w') as f:
...
print(f'saved {OUTDIR}/n_vs_time_gaussian.txt')
...
ax.set_title('Render time vs N (Gaussian)')
ax.legend()
plt.savefig(f'{OUTDIR}/n_vs_time_gaussian.png', bbox_inches='tight', dpi=150)
plt.close(fig)
print(f'saved {OUTDIR}/n_vs_time_gaussian.png')