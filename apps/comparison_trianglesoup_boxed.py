...
#
# Comparison-script version of trianglesoup_rendering_boxed.py, using
# the C++ TriangleSoupBoxedRenderFunction. Core primitive script -- part
# of the four-way comparison (Wendland / Gaussian / Shepard / Triangle
# Soup) used in the report.
#
# Deliberately uses plain uniform-random triangle initialization
# (independent random vertices, no edge-weighting), matching the plain
# random initialization used by every other primitive in the core
# comparison. trianglesoup_rendering_boxed.py's edge-weighted
# initialization is a separate, later improvement reported as its own
# ablation -- it is intentionally NOT used here, so Triangle Soup's
# results stay directly comparable to Wendland/Gaussian/Shepard's.
#
# Usage:
#   python comparison_trianglesoup_boxed.py --target imgs/level_0.png --degraded imgs/level_1.png \
#       --outdir results/comparison_trianglesoup_boxed --n 1000 --iters 200
#
# Args:
#   --target    sharp target image path
#   --degraded  degraded/blurred image, used as starting canvas
...
# opacity (neither fully opaque nor invisible) -- either extreme gives
# weak initial gradients.
...
# --------------------------------------
# Optimization loop. Composited onto the degraded image, same as the
...
ax.set_title(f'Convergence (N={N} triangles, boxed/tile-grid, learnable opacity, '
             f'softness {SOFTNESS_START}->{SOFTNESS_END}px, composited on blurry image)')
...
# Triangles-only render (blank background instead of degraded) -- shows
# raw triangle coverage without the composited canvas underneath
blank_canvas = torch.zeros_like(degraded)
...
# Both error panels share one colour scale so brightness is directly comparable.
