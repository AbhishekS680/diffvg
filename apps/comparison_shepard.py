...
#
# Fits N Shepard IDW control points directly against a sharp target
# image (no degraded starting canvas -- Shepard's global interpolation
# has no defined behaviour when composited over a fixed background, so
# unlike the ellipse-kernel and triangle-soup comparison scripts, this
# one always starts from scratch). Reports SSIM/LPIPS-ready error
# metrics against the target for use in the quantitative comparison
# tables.
#
...
#
# Usage:
#   python comparison_shepard.py --target imgs/level_0.png --degraded imgs/level_1.png \
#       --outdir results/comparison_shepard --n 1000 --iters 200
#
# Args:
#   --target    sharp target image path
#   --degraded  degraded/blurred reference image (used only for the baseline error comparison)
...
parser.add_argument('--degraded', required=True, help='Degraded/blurred reference image path')
...
# Shows how much error exists before the optimizer does anything, so
# the reconstruction's improvement (or lack of it) can be judged against it.
...
# --- Initialize control points and colours ---
...
# --------------------------------------
# Optimization loop: points fit directly against the sharp target.
# --------------------------------------
...
# comparable between "before" and "after."
